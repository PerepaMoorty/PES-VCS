...
// index_load is defined in index.c, but test_tree does not link index.o.
// We provide a local implementation here so tree.c is self-contained
// when built as part of test_tree.
// When linked into the full 'pes' binary, the linker uses index.c's version.
__attribute__((weak))
...
static int write_tree_recursive(const IndexEntry *entries, int count,
    ...
    while (i < count) {
        ...
        if (!slash) {
            ...
            // Subdirectory: extract its name (e.g. "src" from "src/main.c")
            ...
            // Recurse to build and write the subtree
            ObjectID sub_id;
            ...
            // Advance past all entries belonging to this subdirectory
            while (i < count &&
